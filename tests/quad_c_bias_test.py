...
def bias_test(dF, x, true_grad, iter, num_reps=50000):
    grads = []
    ...
    bias_rep = np.cumsum(grads, axis=0)/np.arange(1,
                                                  num_reps+1)[:, np.newaxis] - true_grad(x)

    exp_bias = np.sum(expected_bias, axis=0)
    fig, axs = plt.subplots(2, 1, figsize=(5, 5), sharex=True)
    axs[0].semilogy(np.abs(bias_rep[:, 0] - exp_bias[0]),
                    label='Empirical bias - expected bias')
    axs[0].semilogy(np.abs(bias_rep[:, 0]), label='Empirical bias')
    ...
    axs[1].semilogy(np.abs(bias_rep[:, 1] - exp_bias[1]),
                    label='Empirical bias - expected bias')
    axs[1].semilogy(np.abs(bias_rep[:, 1]), label='Empirical bias')
