"""
Sensitivity sweep for the maximum index-set cardinality (Reviewer request).

Run SGD-MICE (All+Clip) on the same d-dimensional quadratic with fixed gradient
budget; sweep max_hierarchy_size (max |L_k|) with standard ε, δ_drop=0.5, δ_rest=0. Output one CSV.

Usage (from repo root):
  python -m experiments.quadratic.run_max_index_sweep [options]

Output:
  - experiments/output/quadratic_max_index_sensitivity.csv  (one row per max_hierarchy_size value)
"""
...
def run_single(
    *,
    ...
    max_hierarchy_size = maximum index set cardinality |L_k| (paper notation).
    ...
    # All+Clip with standard ε, δ_drop=0.5, δ_rest=0; vary max_hierarchy_size per sweep.
    policy = DropRestartClipPolicy(
        drop_param=0.5,
        restart_param=0.0,
        max_hierarchy_size=int(max_hierarchy_size),
        aggr_cost=0.1,
        clip_type="all",
    )
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Quadratic max index-set cardinality sensitivity sweep with All+Clip (fixed budget)"
    )
    ...
    p.add_argument("--runs", type=int, default=50, help="Number of runs per max_hierarchy_size value")
    p.add_argument("--seed", type=int, default=1, help="Base seed (per-run seed = seed + run_index)")
    p.add_argument(
        "--max-index-values",
        type=str,
        default="100, 500, 1000",
        help="Comma- or space-separated max_hierarchy_size values (default: 20,50,100,200,500)",
    )
    p.add_argument(
        "--outdir",
        type=str,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Output CSV path (default: <outdir>/quadratic_max_index_sensitivity.csv)",
    )
    args = p.parse_args()
    ...
    max_index_values = parse_ints(args.max_index_values)
    if not max_index_values:
        raise ValueError("No max index values. Use --max-index-values to specify at least one value.")

    if args.outdir:
        outdir = Path(args.outdir)
        outdir.mkdir(parents=True, exist_ok=True)
        csv_path = outdir / "quadratic_max_index_sensitivity.csv"
    elif args.csv:
        csv_path = Path(args.csv)
        csv_path.parent.mkdir(parents=True, exist_ok=True)
    else:
        csv_path = Path("quadratic_max_index_sensitivity.csv")

    fieldnames = [
        "max_hierarchy_size", "dim", "kappa", "budget", "n_runs",
    ...
    rows = []
    for max_hierarchy_size in max_index_values:
        runs_data = []
        print(
            f"\nSweep max |L_k|: max_hierarchy_size={max_hierarchy_size} (dim={args.dim}, kappa={args.kappa}, "
