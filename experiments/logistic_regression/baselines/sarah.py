...
from .optimizers import SARAH


def sgd_sarah(dataset, batchsize, Lambda, epochs, seed=0):
    print(f"Solving logistic regression of {dataset} dataset using SARAH")
    ...
    out_dir = dataset_dir / "sarah" / str(seed)
    ...
    while not dF.force_exit:
        k += 1
        grad = dF.evaluate(W[-1], k) + Lambda * W[-1]
        if dF.force_exit:
            break
...
def main() -> int:
    ...
    sgd_sarah(args.dataset, args.batchsize, lambda_reg, epochs, seed=args.seed)
    return 0
