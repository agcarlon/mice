...
from .optimizers import SAG


def sgd_sag(dataset, batchsize, Lambda, epochs, seed=0):
    print(f"Solving logistic regression of {dataset} dataset using SAG")
    ...
    out_dir = dataset_dir / "sag" / str(seed)
    ...
    dF = SAG(lossgrad, sample=thetas_mat, max_cost=epochs * datasize, batchsize=batchsize, verbose=False)

    if n_features < datasize:
        stepsize = 1 / (16 * (L + Lambda * datasize))
    else:
        stepsize = 1 / L
    k = 1
...
def main() -> int:
    ...
    sgd_sag(args.dataset, args.batchsize, lambda_reg, epochs, seed=args.seed)
    return 0
